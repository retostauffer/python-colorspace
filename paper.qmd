...
Python _colorspace_ is an implementation of the _R_ package
`colorspace` [@Zeileis:2020]. 
...
`#FF0000` (red), `#00FF00` (green), and `#0000FF` (blue).
...
be deployed with existing plotting or graphics libraries.
for `matplotlib` (and packages building on it), _colorspace_ provides
functionality for converting HCL-based palettes into compatible
color maps (`cmap`s; `LinearSegmentedColormap`s).

Below, three examples can be found for `matplotlib`, `seaborn`, and `plotly`,
using the data "Harz Traffic" data set shipped with the package (requires
`pandas` to be installed). This real-life data set provides information about
the number of vehicles (separated in bikes, cars, trucks, and other vehicles)
as well as the prevailing weather conditions on a daily basis.


#### Matplotlib

A two-dimensional histogram (joint density) of the daily minimum and maximum
temperature observed. The custom HCL-based palette is provided as 'cmap'
(method `.cmap()`). Whilst the left supblot uses the sequential HCL-based color
palette "ag_Sunset" (reversed), the right plot shows the same data using the
"Blues 2" palette.
...
The following `seaborn`-based barplot uses seven colors from the
qualitative HCL-based color map "Harmonic", showing the 
the average number of bikes per day of week (with error bars)
during summer seasons (June, July, August) 2021 to 2023.

```{python seaborn_minimal}
#| label: fig-seaborn
#| fig-cap: Example on how to use HCL-based palettes with `seaborn`; average number of bikes per day of week during summer season.
...
![Figure based on the graphing library `plotly` using HCL-based colors; Daily mean temperature given Julian day for three consecutive days.\label{fig-plotly}](paper_assets/fig_plotly.svg)
