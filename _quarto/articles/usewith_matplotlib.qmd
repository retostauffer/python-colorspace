...
the 'HarzTraffic' example data set. Visualized is the daily number of bikes
(motorbikes; y-axis) recorded on a public road in Sonnenberg in the Harz region
(Germany) against the average daily mean temperature (x-axis).
...
colors ($\text{Hue} = \pm 180$) correspond to winter time and reddish colors ($\text{Hue} = 0$)
to summer time.
...
maximum temperatures measured at a meteorological station. The
following two-dimensional histogram shows the heatmap
(joint density) betweeen those two variables using
two sequential HCL-based 'cmap's (`LinearSegmentedColormap`s) 
