...
_colorspace_ provides an [interactive app](articles/choose_palette.qmd) for
selecting colors and palettes, leveraging the
[`tkinter`](https://docs.python.org/3/library/tkinter.html) interface which is
part of Python's standard library. However, it is not installed by
default on all systems, particularly on certain Linux distributions.

If [`tkinter`](https://docs.python.org/3/library/tkinter.html) is missing,
_colorspace_ will raise a `RuntimeError` when the feature requiring it
([`choose_palette`](man/colorspace.choose_palette.choose_palette.qmd)) is
invoked. To resolve this, please follow the instructions on [Installing
Tk](https://tkdocs.com/tutorial/install.html) to install
[`tkinter`](https://docs.python.org/3/library/tkinter.html) on your system.

_Note:_ If you're using a virtual environment, you may need to recreate it after
installing [`tkinter`](https://docs.python.org/3/library/tkinter.html) to
